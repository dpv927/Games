...
void MovePlayer(struct Player* player, Vector2* targetPos) {
  float directionX;
  float directionY;
  ...
  targetPos->x += directionX;
  targetPos->y += directionY;

  // Initerpolate player position (smooth)
  player->body.x += PLAYER_LERP * (targetPos->x - player->body.x);
  player->body.y += PLAYER_LERP * (targetPos->y - player->body.y);
...
int main(void) {
  ...
  // Initialize player propierties
  struct Player player;
  ApplyConstantsScale(0.6f);
  InitializePlayer(&player);

  // Target position vextor for 
  // interpolation
  Vector2 targetPos = {0};
  targetPos.x = player.body.x;
  targetPos.y = player.body.y;
  
  while (!WindowShouldClose()) {
    BeginDrawing();
    ClearBackground(WINDOW_COLOR);

    MovePlayer(&player, &targetPos);
    DrawPlayer(player);
    EndDrawing();
  }
